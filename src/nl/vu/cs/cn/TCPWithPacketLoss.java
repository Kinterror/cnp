...
/**
 * This class represents a copy of the TCP stack for test purposes. It should be built on top of the IP stack
 * which is bound to a given IP address.
 * 
 * Note that it is a duplicate of the TCP class, except that it can be passed an integer array which indicates that packets with
 * a certain ID are always dropped.
 */
public class TCPWithPacketLoss {
	...
	/* 
	 * indicates the packets that should be lost. Each field of this array indicates that the packet with that number will
	 * be lost.
	...
	private boolean shouldBeDiscarded(Packet ipPacket){
		for (int i : lostPacketNumbers){
			if (i == ipPacket.id)
				return true;
		}
		return false;
	 ...
	 */
	public TCPWithPacketLoss(int address, int[] lostPacketNumbers) throws IOException {
		ip = new IP(address);
		ip_packet_id = 0;
		timeout = DEFAULT_TIMEOUT;
		this.lostPacketNumbers = lostPacketNumbers;
	}
	...
	void send_tcp_segment(IpAddress destination, TCPSegment p) throws IOException{
		...
		if (!shouldBeDiscarded(ip_packet)){
			//send packet
			ip.ip_send(ip_packet);
		} else {
			//discard packet
			Log.d("send_tcp_segment()", "Drop Packet " + ip_packet.id);
		}
	}
